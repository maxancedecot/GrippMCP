...
export function DashboardViewTabs({ view, params, days, siteId, customPeriod, managers = [], selectedManager = "", clearFilterHref,
  ...
  periodStart?: string; periodEnd?: string; maxDate?: string;
}) {
  return <>
    {view === "campaigns" ? <AccountManagerSidebarFilter managers={managers} selectedManager={selectedManager}
      period={customPeriod ?? (days !== 30 ? { days } : undefined)} clearFilterHref={clearFilterHref}
      periodStart={periodStart} periodEnd={periodEnd} maxDate={maxDate} /> : null}
    <div className="dashboard-sidebar-analytics">
